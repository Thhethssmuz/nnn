...
var applyMiddleware = function (root, middleware, req, res, callback) {
  ...
  var m = middleware.pop().split('/');
  applyMiddleware(root, middleware, req, res, function () {
    root.dispatch(m, req, res, [], [callback]);
  });
};
var callHandler = function (root, handler, req, res, args, args2) {
  // slice used to make a copy of handler.middleware.
  applyMiddleware(root, handler.middleware.slice(), req, res, function () {
    try {
      var a = args2 ? args.concat(args2) : args;
      handler.handler.apply(handler.handler, [req, res].concat(a));
    } catch (err) {
...
var matchHandler = function (router, req, res, args, args2) {
  var method;
  ...
  if (router.methods.hasOwnProperty(req.method))
    ...
    return router.backtrack([], req, res, args, args2);
  else
    return router.root.dispatch(['404'], req, res, []);

  return callHandler(router.root, router.methods[method], req, res, args, args2);
};
var matchQueryHandler = function (router, req, res, args, args2) {
  if (!router.hasOwnProperty('queries'))
    return matchHandler(router, req, res, args, args2);

  for (var i in router.queries) {
    if (router.queries[i].match(req)) {
      args = args.concat(router.queries[i].parseArgs(req));
      return callHandler(router.root, router.queries[i], req, res, args, args2);
    }
  }
...
Router.prototype.dispatch = function (pathArray, req, res, args, args2) {
  var self = this;
  var path = pathArray.shift();

  if (path === undefined)
    return matchHandler(self, req, res, args, args2);

  if (path === '?')
    return matchQueryHandler(self, req, res, args, args2);

  if (self.children.hasOwnProperty(path))
    return self.children[path].dispatch(pathArray, req, res, args, args2);

  else if (self.children.hasOwnProperty(':') && path.length > 0) {
    args.push(path);
    return self.children[':'].dispatch(pathArray, req, res, args, args2);
  }
...
Router.prototype.backtrack = function (pathArray, req, res, args, args2) {
  var self = this;

  if (self.children.hasOwnProperty('*') && pathArray.length > 0) {
    args = ['/' + pathArray.join('/')];
    return self.children['*'].dispatch([], req, res, args, args2);
  }

  else if (self.parent) {
    var path = self.name === ':' ? args.pop() : self.name;
    return self.parent.backtrack([path].concat(pathArray), req, res, args, args2);
  }
