...
var callHandler = function (root, handler, req, res, args) {
  try {
    applyMiddleware(root, handler.middleware.slice(), req, res, function (req, res) {
      try {
...
var matchHandler = function (router, req, res, args) {
  ...
  if (router.hasOwnProperty(req.method))
    method = req.method;
  else if (router.hasOwnProperty('ALL'))
    method = 'ALL';
  ...
  return callHandler(router.root, router[method], req, res, args);
};
var matchQueryHandler = function (router, req, res, args) {
  ...
  for (var i in router.queries) {
    if (router.queries[i].match(req)) {
      ...
      var handler = router.queries[i].getHandler();
      return callHandler(router.root, handler, req, res, args);
    }
  }
  return matchHandler(router, req, res, args);
};
...
Query.prototype.getHandler = function () {
...
Router.prototype.backtrack = function (pathArray, req, res, args) {
  ...
  if (self.children.hasOwnProperty('*') && pathArray.length > 0) {
    args.push('/' + pathArray.join('/'));
    return self.children['*'].dispatch([], req, res, args);
  }

  else if (self.parent)
    return self.parent.backtrack([self.name].concat(pathArray), req, res, args);
