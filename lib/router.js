...
class Router {
  ...
  concat(r) {
    return new Router((s, ok, err) => {
      const nerr = () => r.unRouter(s, ok, err);
  ...
  chain(f) {
    return new Router((s, ok, err) => {
      const nok  = (x, s_) => f(x).unRouter(s_, ok, err);
      return this.unRouter(s, nok, err);
    });
  }
  ...
  static segment(k) {
    return new Router((s, ok, err) => {
      const newstate = s.clone();
      const x = newstate.path.shift();
  ...
  static query(k, v) {
    return new Router((s, ok, err) => {
      return s.queries.reduceRight((r, q, i) => {
        return k(q[0]).chain(x => v(q[1]).chain(y => new Router((s_, ok_) => {
          const newstate = s_.clone();
          newstate.queries.splice(i, 1);
          return ok_([x, y], newstate);
        }))).concat(r);
