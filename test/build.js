...
const show = function (route) {
  return (route.method || 'ALL') + ' ' +
         route.url +
         (route.header ? ' ' + JSON.stringify(route.header) : '');
};
...
test('duplicate handlers', async function (t) {
  ...
  const eq = async (x, y) => {
    const msg = '`' + show(x) + '` equals `' + show(y) + '`';
    return t.throws(() => {
      build([x, y], {});
    }, /duplicate route/, msg);
  };
  const ne = async (x, y) => {
    const msg = '`' + show(x) + '` not equals `' + show(y) + '`';
...
test('case conflicts', async function (t) {
  ...
  const eq = async (x, y, c) => {
    const msg =
      'case ' + (c ? 'in' : '') + 'sensitive `' + show(x) +
      '` conflicts with `' + show(y) + '`';
    return t.throws(async () => {
      build([x, y], {case: c});
    }, /case conflict/, msg);
  };
  const ne = async (x, y, c) => {
    const msg =
      'case ' + (c ? 'in' : '') + 'sensitive `' + show(x) +
      '` does not conflict with `' + show(y) + '`';
    return t.notThrows(async () => {
      build([x, y], {case: c});
    }, msg);
  };
  ...
  await eq(
    {url: '/', header: {'a': 'a', 'b': 'b'}},
    {url: '/', header: {'a': 'A', 'b': 'b'}}, true);
  await ne(
    {url: '/', header: {'a': 'a', 'b': 'b'}},
    {url: '/', header: {'a': 'A', 'b': 'b'}}, false);
  await ne(
    {url: '/', header: {'a': 'a', 'b': 'b'}},
    {url: '/', header: {'a': 'a', 'c': 'c'}}, true);
  await eq(
    {url: '/', header: {'a': 'a', 'b': 'b'}},
    {url: '/', header: {'a': 'A', 'c': 'c'}}, true);
  await ne(
    {url: '/', header: {'a': 'a', 'b': 'b'}},
    {url: '/', header: {'a': 'A', 'c': 'c'}}, false);
  ...
  await ne(
    {url: '/', header: ['a', 'b']},
    {url: '/', header: ['A', 'c']}, false);
});

test('trim conflicts', async function (t) {
  t.plan(7);

  const eq = async (x, y, trim) => {
    const msg =
      (trim ? '' : 'non ') + 'trimming `' + show(x) +
      '` conflicts with `' + show(y) + '`';
    return t.throws(async () => {
      build([x, y], {trim});
    }, /duplicate route/, msg);
  };
  const ne = async (x, y, trim) => {
    const msg =
      (trim ? '' : 'non ') + 'trimming `' + show(x) +
      '` does not conflict with `' + show(y) + '`';
    return t.notThrows(async () => {
