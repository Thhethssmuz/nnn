...
test('start/stop', async function (t) {
  ...
  await t.throws(async () => {
    await app.start({});
  }, /missing server config/, 'missing config');
  ...
  await t.throws(async () => {
    await app.stop();
  }, /Not running/i, 'cannot stop not running server');
});
...
test('http dispatch', async function (t) {
  ...
  app.on('/', async function (...args) {
    this.res.end(JSON.stringify({handler: '/', args}));
  });
  app.on('/test/(\\d+)', async function (...args) {
    this.res.end(JSON.stringify({handler: '/test/(\\d+)', args}));
  });
  app.on('/test?page', async function (...args) {
    this.res.end(JSON.stringify({handler: '/test?page', args}));
  });
  app.on('/throws', async function () {
    throw new Error('some error');
  });
  app.on('/static/**', async function (...args) {
    this.res.end(JSON.stringify({handler: '/static/**', args}));
  });
  app.catch.on(404, async function (...args) {
    this.res.statusCode = 404;
    this.res.end(JSON.stringify({catch: '404', args}));
  });
  ...
  const r1 = await request(`http://localhost:${port}`, {timeout: 1000});
  t.eq(JSON.parse(r1.body), {handler: '/', args: []}, 'root handler matched');

  const r2 = await request(
    `http://localhost:${port}/test/123`, {timeout: 1000});
  t.eq(JSON.parse(r2.body), {handler: '/test/(\\d+)', args: ['123']},
       '/test/123 matched');

  const r3 = await request(
    `http://localhost:${port}/test?page=1337`, {timeout: 1000});
  t.eq(JSON.parse(r3.body), {handler: '/test?page', args: ['1337']},
       '/test?page=1337 matched');

  const r4 = await request(`http://localhost:${port}/lol`, {timeout: 1000});
  t.eq(JSON.parse(r4.body), {
    catch: '404',
    args : [{code: 404, message: 'Not Found', name: 'HttpError'}]
  }, '/lol caught by 404 handler');

  const r5 = await request(`http://localhost:${port}/throws`, {timeout: 1000});
  t.eq(JSON.parse(r5.body), {catch: '500', err: 'some error'},
       '/throws caught by error handler');

  const r6 = await request(
    `http://localhost:${port}/static/my/resource`, {timeout: 1000});
  t.eq(JSON.parse(r6.body), {handler: '/static/**', args: ['my/resource']},
       '/static matched');
...
test('https', async function (t) {
  ...
  const port = app.httpsServer.address().port;
  await t.notThrows(
...
test('app failures', async function (t) {
  ...
  const port1 = app1.httpServer.address().port;
  const r1 = await request(`http://localhost:${port1}`, {timeout: 1000});
  ...
  const r2 = await request(`http://localhost:${port2}`, {timeout: 1000});
  ...
  const r3 = await request(`http://localhost:${port3}`, {timeout: 1000});
